...
// ============================================================================
// TEKNİK ÇİZİM / BELGELER MODÜLÜ — Manifest ve AI içe-aktarma sözleşmeleri
// (bkz. dokumanlar/02-BELGELER-CIZIM-MODULU-TASARIM.md). DEĞİŞTİRME, kullan.
// ============================================================================

export type BelgeKategori =
  | 'mimari' | 'statik' | 'elektrik' | 'mekanik'
  | 'metraj' | 'ruhsat' | 'foto' | 'diger';

export const BELGE_KATEGORI_ETIKET: Record<BelgeKategori, string> = {
  mimari: 'Mimari', statik: 'Statik', elektrik: 'Elektrik', mekanik: 'Mekanik',
  metraj: 'Metraj', ruhsat: 'Ruhsat / Resmi', foto: 'Saha Foto', diger: 'Diğer',
};

export type DosyaTur = 'pdf' | 'dwg' | 'dxf' | 'resim' | 'ofis' | 'diger';

export interface ManifestDosya {
  id: string;
  ad: string;
  dosyaAdi: string;
  yol: string;
  ...
  tarih: string;
  notlar?: string;
  durum?: 'ham' | 'cevrildi' | 'cevrilemedi';
}

export interface BelgelerManifest {
  uretim: string;
  surum: number;
  dosyalar: ManifestDosya[];
}

export interface IceAktarPaket {
  kaynak: string;
  uretim: string;
  not?: string;
  isKalemleri?: Array<Omit<IsKalemi, 'id'>>;
  teklifler?: Array<Omit<Teklif, 'id'>>;
  taseronlar?: Array<Omit<Taseron, 'id'>>;
}
